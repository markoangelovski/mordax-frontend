...
interface SelectInputPropsBase {
  label: string;
  placeholder?: string;
  className: string;
  currentField: string;
...
interface SelectInputProps extends SelectInputPropsBase {
  // label: string;
  // placeholder?: string;
  // className: string;
  // currentField: string;
...
interface SelectProductInputProps extends SelectInputPropsBase {
            ...
            <div className="max-h-96 overflow-auto">
              {data.map((item, i) => (
                <span
                  ...
                  className="flex h-10 cursor-pointer items-center items-center py-2.5 pl-3 pr-3 hover:bg-gray-100"
                  onClick={() => {
                    setCurrentField({ id: item.id, url: item.url });
                    setIsOpen(false);
                  }}
                >
                  {item.url}
